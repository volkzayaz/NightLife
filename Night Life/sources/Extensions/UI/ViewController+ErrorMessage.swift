...
extension UIViewController {
    ...
    /*var viewModel: OriginalViewModel? {
        get {
    ...
    public override class func initialize() {
        ...
        //if self is ErrorHandler{
            
            dispatch_once(&Static.token) {
                let originalSelector = #selector(UIViewController.viewDidLoad)
                let swizzledSelector = #selector(UIViewController.nsh_viewDidLoad<ErrorHandler>())
    ...
    func nsh_viewDidLoad<T : ErrorHandler>() {
        
        self.nsh_viewDidLoad()
        
        guard let new = self as? T else {
            return
        }
        
        new.viewModel.errorMessage.asDriver()
            .filter { $0 != nil }.map { $0! }
    ...
    func simple<T : ErrorHandler> (x : T) -> T{
        return x
    }
    
    func si<T : protocol<ErrorHandler>>(x : T){
        var hh : [T]
    }
